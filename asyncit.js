var AsyncIt = (function(){
    
    var timeOutInterval = 30000,
    
    Interval = function(result, callback, intervalNeed){
        if(typeof(intervalNeed) !== "undefined" && intervalNeed === false)
            return false;
                    ...
                    (function(k){
                        return function(data){
                            ...
                            if(counter === funcs_length){
                                if(!empty(interval))interval.clearInterval();
                                
                                if( (!empty(interval) && interval.checkInterval() === false) || empty(interval) )
